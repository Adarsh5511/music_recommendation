...
function Songs() {
    ...
    useEffect(()=>{
        const playMusic=async ()=>{
           ...
           if(!details.isSet){
                const urlResponse1=await fetch(`https://v1.nocodeapi.com/0darsh/spotify/GMgzDqJiEnXRDPkG/search?q=${localStorage.getItem('songName')}&type=playlist`)
                const convertedResponse1=await urlResponse1.json() 
    
                const urlResponse=await fetch(`https://v1.nocodeapi.com/0darsh/spotify/GMgzDqJiEnXRDPkG/search?q=${localStorage.getItem('songName')}&type=track`)
